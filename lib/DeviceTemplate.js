...
DeviceTemplate.prototype.setAutoUpdate = function (updateFn, timer) {
    let _updater;

    this.gateway.Bridge._server.on('verify', () => {
        console.log(this.name + ' verify');
        if (_updater) {
    ...
    this.gateway.Bridge._server.on('unpair', (username, callback) => {
    ...
    this.updaters.push(setInterval(() => {
        try {
            motion_getter((err, result)=>{
                ...
                auto_update_debug(`${this.gateway.mac}:${this.name} Characteristic.MotionDetected update: ${result}`);
            });
        } catch (e) {
            debug('UpdateFailure')(name + ' update failed.');
        }
