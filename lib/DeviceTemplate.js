    ...
    let pollRequestListener = () => {
        this.gateway.getNodeValues(this.node.id, (err, result) => {
            ...
            setTimeout(() => { // keep cache
                auto_update_debug('Remove cache data.');
                this.cache = null;
...
DeviceTemplate.prototype.getValues = function (callback) {
    if (this.cache) {
        ...
        this.emitter.once('poll-response', (err, result) => { // wait for response;
            auto_update_debug('get response.');
            callback(err, result);
        });
...
DeviceTemplate.prototype.DoorSensor = function () {
    ...
    let getter = (callback) => {
        this.getValues((err, values) => {
            let result = jsonQuery(`[*instance=1][*index=0][*label=Access Control]`, {data:values}).value;
            result = result[0].$t;
    ...
    // setting battery
    let battery = this.accessory.addService(Service.Battery, name);
    let battery_getter = (callback) => {
        this.getValues((err, values) => {
            let result = jsonQuery(`[class=Battery][*instance=1][*index=0][*label=Battery level]`, {data:values}).value;
            result = result[0].$t;
