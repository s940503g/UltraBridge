...
DeviceTemplate.prototype.FourInOneSensor = function () {
    ...
    battery_service.getCharacteristic(Characteristic.LowBatteryStatus).on('get', low_battery_getter);
...
DeviceTemplate.prototype.MiLock = function () {
    let lock_service = this.accessory(Service.LockMechanism, this.name);
    let battery_service = this.accessory(Service.BatteryService, this.name);
    ...
    lock.getCharacteristic(Characteristic.LockTargetState).on('set', lock_target_setter);
