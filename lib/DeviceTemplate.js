...
DeviceTemplate.prototype.setPollRequestListener = function () {
    let pollRequestListener = () => {
        if (this.cache) {
            ...
            this.emitter.emit('poll-response', this.cache);
        } else {
...
DeviceTemplate.prototype.pollRequestListener = function () {
    if (this.cache) {
        auto_update_debug('Use cache data.');
        this.emitter.emit('poll-response', this.cache);
    } else {
        ...
        setTimeout(() => { // keep cache
            auto_update_debug('Remove cache data.');
            this.cache = null;
            this.emitter.once('poll-request', this.pollRequestListener);
        }, 0.05 * 1000);
    }
};
