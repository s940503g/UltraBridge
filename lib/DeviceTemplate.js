...
DeviceTemplate.prototype.MiLock = function () {
    ...
    let lock_target_setter = (state, callback) => {
        setTimeout(() => {
            if (lock_service.getCharacteristic(Characteristic.LockCurrentState).value !== state) {
                lock_service.updateCharacteristic(Characteristic.LockTargetState, state);
            }
        }, 5 * 1000);
...
DeviceTemplate.prototype.ColourLed = function () {
    ...
    let switch_getter = (callback) => {
        this.getValues((err, values) => {
            try {
                var result = jsonpath.query(values, `$..[?(@.instance==1 && @.index==0 && @.label=="Switch")]`)[0].$t;
                result = result == 'True';
                callback(null, result);
            } catch (e) {
                callback(e);
    ...
    let bright_getter = (callback) => {
        this.getValues((err, values) => {
            try {
                var result = jsonpath.query(values, `$..[?(@.instance==1 && @.index==0 && @.label=="SWITCH MULTILEVEL")]`)[0].$t;
                callback(null, result);
            } catch (e) {
                callback(e);
            }
