...
exports.GeneralThermostat_v2 = function () {
    ...
    let state_getter = (callback) => {
		...
		switch (result) {
		    case 'Off': 
			result = Characteristic.CurrentHeaterCoolerState.OFF;
			break;
		    case 'Cool':
			result = Characteristic.CurrentHeaterCoolerState.COOl;
			break;
		    case 'Heat':
			result = Characteristic.CurrentHeaterCoolerState.HEAt;
			break;
    ...
    let temp_getter = (callback) => {
		...
		switch (thermostat_service.getCharacteristic(Characteristic.CurrentHeatingCoolingState).value) {
	        ...
	        var result = jsonpath.query(values, `$..[?(@.instance==1 && @.index==2 && @.class=="THERMOSTAT SETPOINT")]`)[0].$t;
		callback(null, result);
	    } catch (e) {
		callback(e)
	    }
	...
	let value = { class: 'THERMOSTAT SETPOINT', genre: 'user', type: 'byte', instance: 1, index: 2 };
    ...
    this.setAutoUpdate(() => {
        try {
            state_getter((err, result) => {
                ...
                auto_update_debug(`${this.gateway.mac}:${this.name} Characteristic.CurrentHeatingCoolingState: ${result}`);
            });
	   
            temp_getter((err, result) => {
                thermostat_service.updateCharacteristic(Characteristic.CurrentTemperature, result);
                auto_update_debug(`${this.gateway.mac}:${this.name} Characteristic.CurrentTemperature: ${result}`);
            });
        } catch (e) {
            debug('UpdateFailure')(e);
        }
