...
DeviceTemplate.prototype.MiLock = function () {
    ...
    let lock_target_setter = (state, callback) => {
        setTimeout(() => {
            if (Characteristic.getCharacteristic(Characteristic.LockCurrentState).value !== state) {
                Characteristic.updateCharacteristic(Characteristic.LockTargetState, state);
            }
        }, 5 * 1000);
