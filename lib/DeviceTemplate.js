    ...
    for (let instance of instances) {
        ...
        let getter = (callback) => {
            this.getValues((err, values) => {
                ...
                try {
                    if (err) throw err;
                    var result = jsonQuery(`[*instance=${instance}][*index=0][*label=Switch]`, {data:values}).value[0].$t;
                    result = result == 'True';
                } catch (e) {
...
DeviceTemplate.prototype.DoorSensor = function () {
    ...
    let getter = (callback) => {
        this.getValues((err, values) => {
            ...
            try {
                if (err) throw err;
                var result = jsonQuery(`[*class=ALARM][*instance=1][*index=6]`, {data:values}).value[0].$t;
    ...
    let battery_getter = (callback) => {
        this.getValues((err, values) => {
            ...
            try {
                if (err) throw err;
                var result = jsonQuery(`[*class=BATTERY][*instance=1][*index=0]`, {data:values}).value[0].$t;
