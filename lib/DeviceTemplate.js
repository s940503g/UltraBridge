...
const jsonpath = require('jsonpath').query;
...
DeviceTemplate.prototype.WallSwitch = function () {
    let instances = new Set(jsonpath(this.node.value, `$..[instance]`));
    ...
    for (let instance of instances) {
        ...
        let getter = (callback) => {
            this.getValues((err, values) => {
                try {
                    if (err) throw err;
                    var result = jsonpath.query(values, `$..[?(@.instance==${instance} && index==0 && label=="Switch")]`).value[0].$t;
                    result = result == 'True';
                } catch (e) {
...
DeviceTemplate.prototype.DoorSensor = function () {
    ...
    let getter = (callback) => {
        this.getValues((err, values) => {
            try {
                if (err) throw err;
                var result = jsonpath.query(values, `$..[?(@.instance==1 && index==6 && class=="ALARM")]`).value[0].$t;
    ...
    let battery_getter = (callback) => {
        this.getValues((err, values) => {
            try {
                if (err) throw err;
                var result = jsonpath.query(values, `$..[?(@.instance==1 && index==0 && class=="BATTERY")]`).value[0].$t;
