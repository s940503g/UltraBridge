    	...
    	if (title.match(/^RE_WHOIS_AVA_ZWAVE#/)) {
    		...
    		if (!info) { // Discover new gateway not on storage.
				...
				if (!this.publishedGateway[mac]) { // this mac haven't published yet;
					...
					this.publishedGateway[mac].setting.acc = acc;
					this.publishedGateway[mac].setting.pwd = pwd;
					this.publishedGateway[mac].setting.ip = ip;
					this.emitter.emit(`renew-${mac}`, ip, acc, pwd);
				}
			}
		}
