...
Operater.prototype.Battery = function (node) {
	...
	return {
		...
		ChargingState: (value) => {
			let service = this.accessory.getService(name, value.instance) || this.accessory.addService(Service.BatteryService, name+this.instance+'ChargingState', value.instance);
			// No zwave instance
		},
		StatusLowBattery: (value) => {
			let service = this.accessory.getService(name, value.instance) || this.accessory.addService(Service.BatteryService, name+this.instance+'StatusLowBattery', value.instance);
			let lowPower = 10
...
Operater.prototype.ContactSensor = function (node) {
	...
	return {
		...
		StatusActive: (value) => {
			let service = this.accessory.getService(name+value.instance, value.instance) || this.accessory.addService(Service.ContactSensor, name+value.instance+'ActiveStatus', value.instance);
