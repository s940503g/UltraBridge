...
var EXEC_OPTIONS = {timeout: 7, killSignal: "SIGKILL"};
		...
		CurrentAmbientLightLevel: function (value) {
			...
			service.getCharacteristic(Characteristic.CurrentAmbientLightLevel)
				.on('get', function (callback) {
					child_process.exec(cmd+'node_detail.cgi -d "fun=load&id='+nodeId+'"', EXEC_OPTIONS, (err, stdout) => {
	...
	return {
		On: function (value, preprocess) {
			...
			service.getCharacteristic(Characteristic.On)
				.on('get', function (callback) {
					child_process.exec(cmd+'node_detail.cgi -d "fun=load&id='+nodeId+'"', EXEC_OPTIONS, function (err, stdout) {
				...
				.on('set', function (On, callback) {
					...
					child_process.exec(cmd+'valuepost.html -d "'+ nodeId+'-'+ value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + state + '"', EXEC_OPTIONS, function (err, stdout) {
						callback(err);
					});
				});
		...
		LockCurrentState: function (value) {
			...
			let update = function () {
				child_process.exec(cmd+'node_detail.cgi -d "fun=load&id='+nodeId+'"', EXEC_OPTIONS, (err, stdout)=>{
					...
					debug('Update current lock state: ' + result);
				});
			}
			service.getCharacteristic(Characteristic.LockCurrentState).once('update', update);
			service.getCharacteristic(Characteristic.LockCurrentState).emit('update');
		...
		LockTargetState: function (value) {
			...
			service.getCharacteristic(Characteristic.LockTargetState)
				.on('set', function (onLock, callback) {
					...
					child_process.exec(cmd+'valuepost.html -d "'+ nodeId+'-'+ value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + _onLock + '"', EXEC_OPTIONS, function (err, stdout) {
	...
	return {
		...
		StatusLowBattery: function (value) {
			...
			let update = function () {
				...
				}, 30000); // check if low battery every 30 min.
			};
			service.getCharacteristic(Characteristic.StatusLowBattery).once('update', update);
			service.getCharacteristic(Characteristic.StatusLowBattery).emit('update');
		}
		...
		Brightness: function (value) {
			...
			service.addCharacteristic(Characteristic.Brightness)
				...
				.on('set', function (brightness, callback) {
					child_process.exec(cmd+'valuepost.html -d "'+ nodeId+'-'+ value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + brightness + '"', EXEC_OPTIONS, function (err, stdout) {
						callback(err);
			...
			let update = function () {
				child_process.exec(cmd+'node_detail.cgi -d "fun=load&id='+nodeId+'"', EXEC_OPTIONS, (err, stdout)=>{
					...
					debug('update brightness: '+brightness);
				});
			}
			service.once('update', update);
			service.emit('update');
		},
		On: function (value) {
		...
		Hue: function (value) {
			...
			service.addCharacteristic(Characteristic.Hue)
				...
				.on('set', function (hue, callback) {
					...
					child_process.exec(cmd+'valuepost.html -d "'+ nodeId+'-'+ value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + RGB + '"', EXEC_OPTIONS, function (err, stdout) {
						callback(err);
					});
				});
		},
		...
		TargetPosition: function (value) {
			...
			service.getCharacteristic(Characteristic.TargetPosition)
				.on('set', function (level, callback) {
					child_process.exec(cmd+'valuepost.html -d "'+ nodeId+'-'+ value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + level + '"', EXEC_OPTIONS, function (err, stdout) {
						// Should I force state checking?
						let timeout = setTimeout(() => {
							clearInterval(check);
...
function getNodeDetail (gateway, nodeId, callback) {
	...
	let opt = {timeout: 7, killSignal: "SIGKILL"};

	child_process.exec(cmd, opt, function (err, stdout) {
