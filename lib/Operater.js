...
function Operater (gateway, accessory) {
	...
	let resetFn = (err) => {
		...
		setInterval(() => {
			if (this.accessory) this.accessory.once('error', (error) => { resetFn(); });
		}, 1000);
	}

	this.accessory.once('error', (error) => { resetFn(); });
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' update light sensor: ' + result);
					}
				});
			}
			service.getCharacteristic(Characteristic.CurrentAmbientLightLevel).once('update', update);
	...
	return {
		On: (value) => {
			...
			let update = (next) => {
				...
				auto_update_debug(this.mac + ":" + service.displayName + ' switch state update');
			}
			service.getCharacteristic(Characteristic.On).once('update', update);
			service.getCharacteristic(Characteristic.On).emit('update');
		}
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout)=>{
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' update current lock state: ' + result);
					}
				});
			}
...
Operater.prototype.Battery = function (node) {
	...
	return {
		BatteryLevel: (value) => {
			let service = this.accessory.getService(name, value.instance) || this.accessory.addService(Service.BatteryService, name, value.instance);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' batteryLevel update: ' + level);
					}
				});
			}
			service.getCharacteristic(Characteristic.BatteryLevel).once('update', update);
		...
		StatusLowBattery: (value) => {
			...
			let update = () => {
				...
				auto_update_debug(this.mac + ":" + service.displayName + ' LowBatteryLevel state update: ' + level <= lowPower);
			};
			service.getCharacteristic(Characteristic.StatusLowBattery).once('update', update);
			service.getCharacteristic(Characteristic.StatusLowBattery).emit('update');
		}
...
Operater.prototype.Door = function (node) {
	let nodeId = node.id;
	let door = this.accessory.getService(Service.Door) || this.accessory.addService(Service.Door);
...
Operater.prototype.ContactSensor = function (node) {
	let nodeId = node.id;
	let contactSensor = this.accessory.getService(Service.ContactSensor) || this.accessory.addService(Service.ContactSensor);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' CurrentRelativeHumidity state update: ' + hum);
					}
				});
			}
			service.getCharacteristic(Characteristic.CurrentRelativeHumidity).once('update', update);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout)=>{
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' StatusLowBattery state update: ' + result <= lowPower);
					}
				});
			}
			service.getCharacteristic(Characteristic.StatusLowBattery).once('update', update);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName +' CurrentTemperature state update: ' + temp);
					}
				});
			}
			service.getCharacteristic(Characteristic.CurrentTemperature).once('update', update);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName +' StatusActive state update: ' + result);
					}
				});
			}
			service.getCharacteristic(Characteristic.StatusActive).once('update', update);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName +' Brightness state update: ' + brightness);
					}
			...
			let update = (next) => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' Lightbulb state update: ' + result);
					}
				});
			}
			service.getCharacteristic(Characteristic.On).once('update', update);
			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						auto_update_debug(this.mac + ":" + service.displayName + ' WindowCovering CurrentPosition state update: ' + level);
					}
				});
			}
			service.getCharacteristic(Characteristic.CurrentPosition).once('update', update);
