...
Operater.prototype.ContactSensor = function (node) {
	...
	return {
		ContactSensorState: (value, preprocess) => {
			...
			this.gateway.updaters.push(setInterval(update, 1000 * 5));
		},
		StatusActive: (value, preprocess) => {
			let service = this.accessory.getService(name+value.instance, value.instance) || this.accessory.addService(Service.ContactSensor, name+value.instance, value.instance);
...
Operater.prototype.Thermostat = function (node) {
	let nodeId = node.id;
	let name = this.accessory.displayName;

	return {
		CurrentHeatingCoolingState: (value) => {
			let service = this.accessory.getService(name, value.instance) || this.accessory.addService(Service.Thermostat, name, value.instance);
