			...
			let update = () => {
				child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout) => {
					...
					try {
						...
						// result = result.mactch(/closed/) !== null ? Characteristic.ContactSensorState.CONTACT_DETECTED : Characteristic.ContactSensorState.CONTACT_NOT_DETECTED
						// result = preprocess(result);
						result = Characteristic.ContactSensorState.CONTACT_DETECTED;
					} catch (e) {
						result = 0;
						this.gateway.emitter.emit('error', e);
					} finally {
