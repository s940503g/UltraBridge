		...
		LockTargetState: (value) => {
			...
			service.getCharacteristic(Characteristic.LockTargetState)
				.on('set', (onLock, callback) => {
					...
					child_process.exec(this.cmd + 'valuepost.html -d "' + nodeId+'-' + value.class.replace(/\s/,'+') + '-' + value.genre + '-' + value.type + '-' + value.instance + '-' + value.index + '=' + _onLock + '"', EXEC_OPTIONS, function (err, stdout) {
