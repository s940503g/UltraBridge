...
Operater.prototype.ContactSensor = function (node) {
	...
	return {
		ContactSensorState: (value, preprocess) => {
			let service = this.accessory.getService(name, value.instance) || this.accessory.addService(Service.ContactSensor, name, value.instance);
			service.getCharacteristic(Characteristic.BatteryLevel)
				.on('get', (callback) => {
					child_process.exec(this.cmd + 'node_detail.cgi -d "fun=load&id=' + nodeId + '"', EXEC_OPTIONS, (err, stdout)=>{
						let result;
						try {
							...
							//result = result.mactch(/opened/) ? Characteristic.ContactSensorState.CONTACT_DETECTED : Characteristic.ContactSensorState.CONTACT_NOT_DETECTED
						} catch (e) {
							result = 0;
							this.gateway.emitter.emit('error', e);
						} finally {
