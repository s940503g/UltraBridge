		...
		switch (value.label) {
			case 'Access Control':
				operater.ContactSensor(node).ContactSensorState(value, function(result) {
					return result = result.match(/closed/) !== null ? Characteristic.ContactSensorState.CONTACT_DETECTED : Characteristic.ContactSensorState.CONTACT_NOT_DETECTED;
				});
				break;
			case 'Battery Level':
				operater.Battery(node).BatteryLevel(value);
				break;
			case 'Mode':
				operater.ContactSensor(node).StatusActive(value);
				break;
		}
	});
	this.emitter.emit('new_accessory', accessory);
