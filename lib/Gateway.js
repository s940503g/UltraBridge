...
Gateway.prototype.bridgeDevice = function (node) {
	...
	try {
		if (productToTemplate[node.product] == "WallSwitch") {
			...
			let device = new Device(DeviceTemplate.ZW4102CurtainControlModule_v2, this, node);
			this.addDevice(device);
		}
		else if (productToTemplate[node.product] == "Curtain_v2") {
			let device = new Device(DeviceTemplate.ZW41115CurtainControlModule, this, node);
			this.addDevice(device);
		}
		else if (productToTemplate[node.product] == "DimmerModule") {
			let device = new Device(DeviceTemplate.DimmerModule, this, node);
			this.addDevice(device);
		}
		else if (productToTemplate[node.product] == "GeneralThermostat_v2") {
			let device = new Device(DeviceTemplate.GeneralThermostat_v2, this, node);
			this.addDevice(device);
		}
		else {
...
Gateway.prototype.addDevice = function (device) {
	...
	try {
		...
		// device.remove();
		console.log('Fail to bridge ' + device.name);
	}
};
...
Gateway.prototype.establishBridgedNodes = function (callback) {
	this.getInfrastructure((err, data) => {
		try {
			...
			}else{ // only one device in gateway
				this.bridgeDevice(data.node_detail.node);
			}
...
Gateway.prototype.setWatchDog = function () {
	this.reachableUpdater = setInterval(() => {
		isReachable(this.setting.ip + ":5000", {timeout: 10 * 1000}).then((reachable) => {
			if (reachable) {
