...
Gateway.prototype.destroy = function () {
	try {
		...
		for (var acc in this.BridgeAccessory.bridgedAccessories) {

			this.BridgeAccessory.bridgedAccessories[acc].removeAllListeners();

			for (var service in this.BridgeAccessory.bridgedAccessories[acc].services) {
				this.BridgeAccessory.bridgedAccessories[acc].services[service].removeAllListeners();
			}
		}
