	...
	this.emitter.on('bridge_node', (node) => {
		...
		switch (node.product) {
			case 'ZL7435 In-Wall Switch, 2 Relays':
				debug('Configuring ZL7435 In-Wall Switch, 2 Relays.');
				this.Bridge.addBridgedAccessory(new DeviceTemplate(this, node).WallSwitch());
				break;
			case "Wall Switch x3":
				debug('Configuring Wall Switch x3.');
				this.Bridge.addBridgedAccessory(new DeviceTemplate(this, node).WallSwitch());
				break;
...
Gateway.prototype.BridgeGateway = function (acc, pwd) {
	...
	this.getInfrastructure((err, data) => {
		try {
			...
			if (!this.Bridge) throw new Error(this.mac + " BridgeAccessory not exists.");
