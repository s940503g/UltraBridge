...
Gateway.prototype.bridgeThermostat = function (node) {
	...
	node.value.forEach((value) => {
		switch (value.label) {
			case 'Battery Level':
				...
				break;
			case 'Mode':
				operater.Thermostat(node).CurrentHeatingCoolingState(value);
				break;
			case 'Cooling 1':
				operater.Thermostat(node).TargetTemperature(value);
				break;
			case 'Temperature':
				operater.Thermostat(node).CurrentTemperature(value);
				break;
		}
	});
	this.emitter.emit('new_accessory', accessory);
