...
function Gateway(mac) {
	...
	// use ipFinder variable to get model infomation.
	this.model = ipFinder.publishedGateway[mac].model;
	console.log(ipFinder.publishedGateway);
...
Gateway.prototype.destroy = function () {
	try {
		if (!this.published) {
			throw  "Gateway" + this.mac + " not publised yet.";
		}
		this.BridgeAccessory.destroy();
		this.emitter.emit('bridge-destroy');
		this.emitter.removeAllListeners(() => {
			debug('Gateway ' + this.mac + ' all listeners are removed.');
		});
		this.published = false;
		debug('Gateway ' + this.mac + ' destroyed.');
	} catch (e) {
		debug(e);
	}
};
...
Gateway.prototype.BridgeGateway = function (acc, pwd) {
	...
	this.emitter.on('bridge_node', (node) => {
		debug('Gateway ' + this.mac + ' bridging ' + node.product);
		switch (node.product) {
...
Gateway.prototype.publish = function (port, pincode) {
	...
	if (this.published) this.destroy();
...
Gateway.prototype.reset = function () {
	...
	setTimeout(() => {
		...
		if (this._info.acc == '' || this._info.pwd == '') {
			this.BridgeAccessory.destroy();
			this.emitter.emit('bridge-destroy');
		}
	}, 20);
