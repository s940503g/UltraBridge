...
Gateway.prototype.publish = function (port, pincode) {
	...
	this.published = true;
	debug('Gateway ' + this.mac + 'publish.');
};
...
Gateway.prototype.trackingStructureChange = function () {
	this.tracking = setInterval(() => {
		this.getInfrastructure((err, data) => {
			if (err || data == {})
				this.resetIp(mac);
			else if (data.node_detail.node.length != this.nodeCount)
				this.reloadNodes(data); // Discover new node

			debug(this.mac + 'current node number: ' + data.node_detail.node.length);
		});
...
Gateway.prototype.resetIp = function (mac) {
	...
	this.setting.ip = ipFinder.gateway_list[mac]._info.ip;
}

Gateway.prototype.reloadNodes = function (data) {
	data.node_detail.node.forEach((node) => {
		this.emitter.emit('bridge_scene', node);
	});

	this.getScenes(function (err, data) {
		if (!err && data.sceneid instanceof Array) {
			debug(data.sceneid);
	...
	this.emitter.emit('publish', this.BridgeAccessory);
}
