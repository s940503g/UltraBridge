	...
	this.tracking = setInterval(() => {
		this.getInfrastructure((err, data) => {
			if (err || data == {})
				...
				tracking_debug('Gateway '+ this.mac + ' current node number: ' + data.node_detail.node.length);
		});
		tracking_debug('Tracking gateway ' + this.mac + ' change.');
	}, 1000 * 10 * 1);
}

Gateway.prototype.resetIp = function () {
	ipFinder.emit(); // rescan gateway on network;
	this.BridgeAccessory._advertiser.stopAdvertising();
	setTimeout(()=>{
	...
	debug('Gateway ' + this.mac + ' reseting ip');
}
